...
async function handleRequest(request, env) {
  ...
  if (path === "/set") {
    ...
    let privateNote = data.private || false;
  ...
  if (path === "/admin") {
    ...
    for (const k of list.keys) {
      ...
      if (meta.expire) {
        expireText = new Date(meta.expire).toLocaleString();
      }
  ...
  if (meta.private) {
    ...
    if (reqToken !== token) {
      return new Response("这是一个私密笔记，需要携带访问 Token 才能查看。\n\n访问方法：在地址后添加 ?token=你的Token\n例如：/访问码?token=你的Token", { status: 403 });
    }
  }
...
<label>
<input type="checkbox" id="private"> 私密笔记
</label>
...
toc+=\`<div style="margin-left:\${(h.tagName[1]-1)*10}px">
